...
fn average(a: f64, b: f64) -> f64 {
    const A_WEIGHT: f64 = 3.5;
    let a = a * A_WEIGHT;

    const B_WEIGHT: f64 = 7.5;
    let b = b * B_WEIGHT;

    (a + b) / (A_WEIGHT + B_WEIGHT)
}
...
mod tests {
    ...
    fn test_solution_a() {
        ...
        assert_eq!(solution(cursor), "MEDIA = 6.43182");
    }

    #[test]
    fn test_solution_b() {
        let cursor = Cursor::new(b"0.0\n7.1\n");
        assert_eq!(solution(cursor), "MEDIA = 4.84091");
    }

    #[test]
    fn test_solution_c() {
        let cursor = Cursor::new(b"10.0\n10.0\n");
        assert_eq!(solution(cursor), "MEDIA = 10.00000");
    }
}
